...
namespace CampeonatosNParty.Helpers
{
    public class EncryptHelper
    {
        ...
        private static string chave = "SESE&)@(23KSJHST";

        /// <summary>
        /// Criptografa uma string no padrao DES3.
        /// </summary>
        /// <param name="texto">Texto puro que ser criptografado.</param>
        /// <returns>Retorna o valor criptografado.</returns>
        public static string CriptografarDES3(string texto)
        {
            TripleDESCryptoServiceProvider des3Crypto = new TripleDESCryptoServiceProvider();

            UTF8Encoding encoder = new UTF8Encoding();

            des3Crypto.Key = encoder.GetBytes(chave);

            des3Crypto.Mode = CipherMode.ECB;

            byte[] buffer = ASCIIEncoding.ASCII.GetBytes(texto);

            string encrypted = Convert.ToBase64String(
                des3Crypto.CreateEncryptor().TransformFinalBlock(buffer, 0, buffer.Length)
            );
